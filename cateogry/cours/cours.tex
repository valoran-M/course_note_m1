...
\begin{document}
  ...
  Free monoide on a set (=alphabet) $A$ $A^*$ contains finite sequences of
  element $A$ $w = [a_1 \ldots a_n]$
  ...
  \define A category $\mathcal C$ is a \underline{graph}
    ...
    \begin{itemize}
      \item an morphisms $id_A \in Home(A, A)$ for all object $A$ of $\mathcal C$
      \item a composition defined as a function $\circ_{A,B,C} : Hom(B, C)
        \times Hom(A, B) \to Hom(A, C)$ for every objects $A,B,C$ of category
        $\mathcal C$
  ...
  \underline{Ordered Set} : Claim every ordered set $A$ defines a category
    ...
    \define An order category $\mathcal C$ is a category when $Hom(A,B)$ is a
